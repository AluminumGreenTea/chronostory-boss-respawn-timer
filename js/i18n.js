...
const STRINGS = {
  ...
  'note.ph':     { zh: '例如：紅王、東洞…', en: 'e.g. Red King, East Cave…' },
  'field.channel': { zh: '頻道', en: 'Channel' },
  'channel.sub': { zh: '一筆 = 一個頻道；多頻道請各自新增', en: 'One entry = one channel; add each separately' },
  'channel.ph':  { zh: '輸入頻道數字，例如 3', en: 'Enter a channel number, e.g. 3' },
  ...
  'err.channel': { zh: '請輸入正確的頻道數字。', en: 'Please enter a valid channel number.' },
  'err.interval':{ zh: '重生間隔需大於 0。', en: 'Respawn interval must be greater than 0.' },

  'card.reset':  { zh: '重置', en: 'Reset' },
  'card.copy':   { zh: '複製', en: 'Copy' },
  'card.edit':   { zh: '編輯', en: 'Edit' },
  'card.delete': { zh: '刪除', en: 'Delete' },
  'ch.title':    { zh: '點擊修改頻道；聚焦後上下鍵 ±1，Shift+上下鍵 ±10',
                   en: 'Click to edit channel; ↑/↓ ±1, Shift+↑/↓ ±10' },
  'ch.aria':     { zh: '頻道 {n}，點擊修改，或用上下方向鍵增減', en: 'Channel {n}, click to edit, or use arrow keys' },

  'filter.all':  { zh: '全部', en: 'All' },
  'toast.undo':  { zh: '復原', en: 'Undo' },
  'toast.deleted': { zh: '已刪除「{who}」', en: 'Deleted “{who}”' },
  'toast.cleared': { zh: '已清除全部 {n} 筆計時器', en: 'Cleared all {n} timers' },
  ...
  'announce.clearRestored': { zh: '已復原 {n} 筆計時器', en: 'Restored {n} timers' },
  ...
  'sound.off':   { zh: '提示音：關（點擊開啟）', en: 'Sound: off (click to unmute)' },
  'theme.dark':  { zh: '深色模式（點擊切換為淺色）', en: 'Dark mode (click for light)' },
  'theme.light': { zh: '淺色模式（點擊切換為深色）', en: 'Light mode (click for dark)' },
};
...
const MapleI18n = {
  ...
  save(l) { try { localStorage.setItem(I18N_KEY, l); } catch (e) { /* 忽略 */ } },
  ...
  monster(mon) { return (this.lang === 'en' && mon && mon.nameEn) ? mon.nameEn : (mon ? mon.name : ''); },
  color(col)   { return (this.lang === 'en' && col && col.nameEn) ? col.nameEn : (col ? col.name : ''); },

  /* 套用到有 data-i18n* 標記的靜態 DOM */
  apply() {
    document.querySelectorAll('[data-i18n]').forEach(el => { el.textContent = this.t(el.dataset.i18n); });
    document.querySelectorAll('[data-i18n-ph]').forEach(el => { el.setAttribute('placeholder', this.t(el.dataset.i18nPh)); });
  ...
  toggle() { this.set(this.lang === 'zh' ? 'en' : 'zh'); },
